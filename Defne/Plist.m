...
+(BOOL)listExists:(NSString *)listName
{
    NSArray *listAll = [self allLists];
    
    listName = [NSString stringWithFormat:@"%@.plist" , listName];
    for(Plist *theList in listAll)
    {
        NSString *nameOfListInFolder = [theList plistNameOfInstance];
        if([nameOfListInFolder isEqualToString:listName])
        {
            return YES;
        }
    }
